...
/** Instantiates class SharedCloudTest. */
public class SharedCloudTest {
   ...
   /** Test method for getAndSetdataStored. **/
   @Test public void getAndSetdataStoredTest() {
      SharedCloud c2 = new SharedCloud("Cloud Two", 9.00, 12.0, 20.0);
      Assert.assertEquals("get data stored failed",
         12.000, c2.getDataStored(), .0001);
      c2.setDataStored(42.00);
      Assert.assertEquals("set data stored failed",
         42.00, c2.getDataStored(), .0001);
   }
   /** Test method for getAndSetdataLimit. */
   @Test public void getAndSetdataLimitTest() {
      SharedCloud c2 = new SharedCloud("Cloud Two",
         9.00, 12.0, 20.0);
      Assert.assertEquals("get data limit failed",
         20.000, c2.getDataLimit(), .0001);
      c2.setDataLimitCloudS(42.00);
      Assert.assertEquals("set data limit failed",
         42.000, c2.getDataLimit(), .0001);
   }
   ...
   @Test public void getCostFactorTest() {
      ...
      Assert.assertEquals("get cost factor failed",
         1.0, c2.getCostFactor(), .0001);
   }
   /** Test method for dataOverage. */
   @Test public void dataOverageTest() {
      SharedCloud c2 = new SharedCloud("Cloud Two",
         9.00, 12.0, 20.0);
      Assert.assertEquals("data overage failed",
         0.000, c2.dataOverage(), .0001);
      SharedCloud c3 = new SharedCloud("Cloud Three",
         9.00, 25.0, 20.0);
      Assert.assertEquals("data overage failed",
         5.000, c3.dataOverage(), .0001);
   }
